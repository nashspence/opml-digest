...
The repository packages a single containerised script, `onefeed.py`. Keep the project aligned with this layout when adding or updating files.
...
1. Format & lint all changes:
   ...
   ```
2. Run dependency-free checks:
   ```
   pytest
   ```
3. Update `spec.md` whenever `onefeed.py` or its CLI surface changes.
...
- Place tests in the `tests/` directory adjacent to `onefeed.py` and mock external services so unit tests exercise only the code under test.
