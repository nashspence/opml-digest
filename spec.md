...
And each feed entry includes url, priority, description, representation, includeComments, maxPosts, maxArticleChars, and maxCommentsChars fields
And the OPENAI_API_KEY environment variable is set
When I run `python onefeed.py /path/to/config.yaml 2025-11-01`
...
And when I run `python onefeed.py --validate /path/to/config.yaml`
Then the config is validated against config-schema.yaml without contacting feeds or OpenAI
And when I run `python onefeed.py --write-example ./my-config.yaml`
